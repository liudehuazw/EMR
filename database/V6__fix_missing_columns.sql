-- ============================================================
--  V6: 补齐代码中引用、但历史脚本遗漏的字段
-- ============================================================
--  适用场景：数据库已存在（已执行过旧版 init.sql / V2 / V3 / V4 / V5），
--            需要在不丢数据的前提下补齐缺失字段。
--
--  用法：mysql -u root -p emr_db < database/V6__fix_missing_columns.sql
--
--  背景：
...
--    注：检验报告数据隔离通过 patient.user_id 实现，emr_lab_report 不含 user_id 列。
--
--  本脚本可重复执行（幂等）。
-- ============================================================
...
WHERE TABLE_SCHEMA = DATABASE()
  AND ((TABLE_NAME = 'emr_lab_report' AND COLUMN_NAME = 'table_data')
