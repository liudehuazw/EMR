...
家庭健康档案分散在纸质报告、相册和各家医院小程序里，未在同一医院固定就诊时难以检索和对比趋势，且很多医院仅保存固定期限的报告。以往，我们常用excel或截图保存就诊资料，现在，我们摒弃这种低效繁琐的方式，本项目把**患者、病历、检验、影像、发票**集中管理，并强调：**默认本地存储 + 本地 OCR**，敏感资料不必先上传到公有云。AI 解读与就诊助手为**可选项**，可接 DeepSeek 等云端 API，也可接 **本地Ollama**。
